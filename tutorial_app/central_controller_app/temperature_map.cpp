...
extern int Output(const char *aBuf, uint16_t aBufLength);
...
{
	Output("VALUE: ", 7);
	std::string str_number = std::to_string(temperature);
	const char* temp_str = str_number.c_str();
	Output(temp_str, strlen(temp_str));
	Output("\r\n",2);
